...
from visualize import visualize_vae,visualize_ldt
import os

def evaluate(model,dataloader,device,epoch=None,visualize=False,save_path=None,ddpm=False):
    if ddpm:
        model.model.eval()
    else:
        model.eval()
...
''' if visualize and epoch is not None:
    ...
    visualize_denoising_process(model, device, denoise_fp, epoch, title=f'denoising_epoch{epoch}', stepToShow=8)
    ...
    visualize_samples(model, samples_fp, num_samples=16, title=f'samples_epoch{epoch}')

    # 4. 对比原始/加噪/去噪图像
    compare_fp = os.path.join(save_path, f'orig_noisy_denoised_epoch{epoch}_size{img_size}.png')
    compare_original_noise_denoised(model, dataloader, device, compare_fp, epoch, title=f'orig_noisy_denoised_epoch{epoch}')
'''
